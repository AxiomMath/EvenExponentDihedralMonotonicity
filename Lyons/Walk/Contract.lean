...
Convolving a group-algebra element `x` by a probability element `κ`, one with nonnegative
coefficients summing to one, cannot increase `∑ g, x_g ^ (2m)`. No sign hypothesis on `x` is
needed, and none is available: the intended `x` is a centred heat element, whose coefficients are
negative wherever the walk sits below uniform. At an even exponent `· ^ (2m)` is convex on all of
`ℝ`, not just on `[0, ∞)`, so Jensen's inequality applies with the coefficients of `κ` as the
weights.
...
/-- The convolution rule with the sum taken over the *left* factor's index. `Lyons.co_mul` sums
over the right factor; this is the same identity reindexed by `h ↦ g * h⁻¹`. -/
...
`κ` is required only to have nonnegative coefficients summing to one — it need not be a heat
element, and `x` is arbitrary. -/
