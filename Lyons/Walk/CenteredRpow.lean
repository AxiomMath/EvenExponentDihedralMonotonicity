...
The centered heat element satisfies `(a_t^λ) ^ θ = a_{θ t}^λ`. It factors as
`a_t^λ = H_t - π_G = H_t Q`, where `Q = 1 - P` is a projection commuting with the heat matrix
`H_t`; the fractional power rescales the time on the exponential factor `H_t`, and the
projection factors out of the functional calculus because `x ^ θ` vanishes at `0`.

The factor `Q` cannot instead be absorbed spectrally, that is, written as `cfc g A` for the
generator `A` and some `g` vanishing at `0`: if the walk is reducible then the kernel of `A` is
strictly larger than the constants, and such a `g` sends the whole kernel to `0`, whereas `Q` is
`1` on the part of the kernel outside `range P`.

## Main results

* `Lyons.cfc_exp_eq_exp`: `cfc Real.exp A = exp A` for Hermitian `A`.
* `Lyons.cfc_rpow_exp`: `cfc (· ^ θ) (exp A) = exp (θ • A)`.
* `Lyons.cfc_mul_proj`: `cfc f (M * Q) = cfc f M * Q` for a projection `Q` commuting with `M`
  and `f` vanishing at `0`.
* `Lyons.powElt_centeredElt`: `(a_t^λ) ^ θ = a_{θ t}^λ`.
-/
...
/-- **The functional calculus factors through a commuting projection**, provided `f` vanishes
at `0`. -/
