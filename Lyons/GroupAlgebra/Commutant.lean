...
The left regular representation of `ℝ[G]` is characterised, inside all of `Matrix G G ℂ`, by
commuting with every right translation. This is the mechanism by which the group algebra is shown
closed under operations defined on matrices — notably the continuous functional calculus, whose
output commutes with everything the input commutes with (`Commute.cfc_real`), hence with every
...
* `Lyons.commutes_R_iff`: a matrix commutes with every right translation iff its entries depend
  only on `g * g'⁻¹`, i.e. iff it is a left convolution.
* `Lyons.cfc_L_conv`: a continuous-functional-calculus image of `L x` is again a left convolution.
-/
...
/-- Right translation by `h`, as a `G × G` matrix: it sends the basis vector at `g` to the one at
`g * h`. -/
...
/-- **The commutant of the right translations.** A matrix commutes with every right translation
exactly when its entries depend only on `g * g'⁻¹` — that is, exactly when it is a left convolution
operator.

The conclusion is the entrywise condition rather than `∃ z, T = L z` because `L` has *real*
coefficients while a general `T` satisfying the right-hand side need not, so the existence form
over `ℝ[G]` would be false. -/
...
/-- The entries of a left convolution matrix are determined by its first column, which is the
...
/-- **The functional calculus does not leave the group algebra.** Applied to a left convolution
`L x`, any continuous-functional-calculus image `cfc f (L x)` is again a left convolution: its
entries depend only on `g * g'⁻¹`. -/
