...
* `Lyons.coe_laplacian`: `Δ_λ` has coefficient `Λ - λ 1` at the identity and `-λ g` elsewhere,
  where `Λ = ∑ s, λ s`.
...
/-- Coefficients of the Laplacian: the total rate at the identity, `-λ g` elsewhere. -/
...
/-- Coefficients of `x * π_G`: every one is the coefficient sum of `x`, divided by `|G|`. -/
...
/-- **Absorption:** `x * π_G` is the coefficient sum of `x` times `π_G`. -/
