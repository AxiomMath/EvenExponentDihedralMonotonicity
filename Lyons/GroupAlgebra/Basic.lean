...
For a finite group `G` we work in `MonoidAlgebra ℝ G` and transport analytic questions along the
**left regular representation**

`L x : Matrix G G ℂ`,  `(L x) g h = x (g * h⁻¹)`.

`L` is an injective `ℝ`-algebra map that intertwines the involution with the conjugate transpose,
so positivity and fractional powers can be imported from `Matrix`, which Mathlib equips with a
`ContinuousFunctionalCalculus` instance, rather than rebuilt on `MonoidAlgebra`.

`MonoidAlgebra` supplies the ring structure. It does **not** carry a `StarRing` instance for a
group, so the involution is defined here.

## Main definitions

* `Lyons.co`: the coefficient of `x` at `g`.
* `Lyons.invol`: the involution `x⋆ g = x g⁻¹` (real coefficients, so no conjugation).
* `Lyons.L`: the left regular representation as a complex matrix.
* `Lyons.ofFun`: the element with a prescribed coefficient function.
* `Lyons.IsPos`: `x ⪰ 0`, meaning `L x` is positive semidefinite.

## Main results

* `Lyons.co_mul`: the convolution rule for the coefficients of a product.
* `Lyons.L_mul`, `Lyons.L_invol`, `Lyons.L_injective`: `L` is a star-preserving injective ring map.
-/
...
`MonoidAlgebra ℝ G` is a structure wrapping `coeff : G →₀ ℝ`, and `co x g` is the coefficient of
`x` at `g`. -/
...
/-- **The convolution rule** for the coefficients of a product in `ℝ[G]`. -/
...
/-! ### Coefficients of the algebra operations -/
...
/-- Expansion of an element over the basis of group elements. -/
...
/-- Build an element from a coefficient function; inverse to `co` on a finite group. -/
...
/-- The involution `x⋆ g = x g⁻¹`. The coefficients are real, so unlike the complex group algebra
...
/-- `L` is injective: the coefficients can be read off the column at the identity. -/
...
/-- A positive element is self-adjoint. -/
