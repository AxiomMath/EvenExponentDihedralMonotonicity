...
For positive `a` and `θ ∈ (0,1)` the sandwich is the group-algebra element whose left regular
matrix is `(L a) ^ θ * L b * (L a) ^ (1 - θ)`. The matrix product commutes with every right
translation and has real entries, so its first column defines a group-algebra element with that
matrix.
...
* `Lyons.L_sandwich`: the left regular matrix of `Lyons.sandwich` is `Lyons.sandwichMat`.
* `Lyons.sandwich_eq_mul`: the sandwich is the product `a ^ θ * b * a ^ (1 - θ)` in the group
  algebra.
-/
...
/-- The sandwich matrix commutes with every right translation. -/
...
/-- The entries of the sandwich matrix depend only on `g * g'⁻¹`. -/
...
/-- The sandwich matrix is fixed by entrywise conjugation, hence has real entries. -/
...
/-- **The sandwich is a product inside the group algebra.** The definition builds it from a matrix
column; this says it is exactly `a ^ θ * b * a ^ (1 - θ)`, with all three factors group-algebra
elements. -/
