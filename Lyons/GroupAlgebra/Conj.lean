...
`Lyons.cfc_L_conv` shows that a functional-calculus image of `L x` is again a left convolution —
its entries are determined by the first column. To read a group-algebra element off that column one
also needs the entries to be **real**, and that is what this file supplies.

Entrywise complex conjugation is a continuous `ℝ`-algebra star endomorphism of `Matrix G G ℂ`
(`Lyons.conjHom`), so Mathlib's `StarAlgHomClass.map_cfc` lets it pass through the continuous
functional calculus. Since `L x` has real entries it is fixed by conjugation, so `cfc f (L x)` is
fixed too — which is exactly realness of its entries.
...
* `Lyons.continuous_conjHom`: `conjHom` is continuous.
...
/-- Entrywise complex conjugation, as an `ℝ`-algebra star endomorphism of the matrix algebra. It is
`ℝ`-linear but not `ℂ`-linear, which is why the scalars must be `ℝ` here. -/
...
/-- **The entries of `cfc f (L x)` are real.** This is what lets a group-algebra element be
reconstructed from the first column. -/
