...
`heatCoeff` is defined as an entry of `exp (-t • L Δ)`, hence lands in `ℂ`, while the main
theorem is about `|p_t(g) - 1/|G||^{2m}` with `p_t(g)` real.

Entrywise conjugation on matrices is `M ↦ (Mᴴ)ᵀ`, so it commutes with the exponential. Since
`L x` has real entries by construction, it is fixed by conjugation, and therefore so is its
exponential — which is exactly realness of the entries. No spectral theory is involved.

## Main definitions

* `Lyons.heatCoeffReal`: the transition function `p_t^λ`, as an `ℝ`-valued function.
* `Lyons.centeredHeatCoeffReal`: `p_t^λ - 1/|G|`, `ℝ`-valued.
...
* `Lyons.heatCoeff_eq_real`: `heatCoeff` is the coercion of `heatCoeffReal`.
* `Lyons.Phi_eq_sum_real`: `Phi` as a real power sum.
-/
...
/-- The transition function, as an honestly `ℝ`-valued function: the `p_t^λ` of the source
paper. -/
@[lyons_tag "def_heat_coeff_real"]
noncomputable def heatCoeffReal (lam : RateFn G) (t : ℝ) : G → ℝ :=
  fun g => (heatCoeff lam t g).re

/-- The `ℝ`-valued and `ℂ`-valued transition functions agree: the heat coefficient is real. -/
...
/-- `Phi` as a real power sum: with real coefficients the norm and the even power agree,
`|x| ^ (2m) = x ^ (2m)`. -/
