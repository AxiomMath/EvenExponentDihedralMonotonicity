...
If `u` has a nonnegative-real Fourier transform dominating that of `f` pointwise, then
`‖f‖_{2m} ≤ ‖u‖_{2m}` for every positive integer `m`. This is the elementary positive half of the
Hardy–Littlewood majorant principle, available precisely because the exponent is an even integer:
`‖f‖_{2m}^{2m} = ‖f ^ m‖_2^2`, so Parseval applies to the pointwise power. Evenness of the
exponent is used only at that step; the remaining ones work for every real `p ≥ 1`.
...
* `ZMod.dft_pow_dominated`: domination of the transforms is inherited by pointwise powers, and
  the transform of the majorant stays a nonnegative real.
...
`z = (‖z‖ : ℂ)` says exactly that `z` is a nonnegative real. -/
...
/-- The transform of a pointwise product is the normalised convolution of the transforms. -/
...
/-- Domination of the transforms is inherited by pointwise powers, and the transform of the
majorant stays a nonnegative real. -/
...
/-- **The finite even-exponent majorant inequality.** If each `𝓕 u k` is a nonnegative real
dominating `‖𝓕 f k‖`, then `‖f‖_{2m} ≤ ‖u‖_{2m}`. -/
