...
The even part of a `ℂ`-valued function on `ZMod N`, the counting `ℓ^p` norm, and the behaviour of
the discrete Fourier transform under negating the frequency.
...
* `ZMod.dft_evenPart`: the transform of the even part is the real part of the transform.
...
/-- For a **real-valued** function, negating the frequency conjugates the transform. -/
...
/-- **The transform determines the function.** Applying the transform twice recovers
`N • f ∘ (-·)`. -/
