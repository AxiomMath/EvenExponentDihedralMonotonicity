...
`Lyons.heatMat` is a matrix and `Lyons.heatCoeff` its first column, and a column cannot be
multiplied by the uniform element, raised to a fractional power, or fed to a second
representation. So this file reconstructs the group-algebra element `h_t^λ` whose coefficient
function is the transition function.

The one analytic input is `Lyons.exp_mul_of_mul_eq_zero`: if `A * B = 0` then `exp A * B = B`.
Since `Δ_λ π_G = 0`, this gives absorption `h_t^λ π_G = π_G h_t^λ = π_G`, and the coefficient sum
`∑ g, p_t^λ(g) = 1` then follows from absorption.

## Main definitions

* `Lyons.totalRate`: `Λ_λ = ∑ s, λ s`.
* `Lyons.jumpElt`: `w_λ = ∑ s, λ s · s`.
...
* `Lyons.laplacian_eq_totalRate_sub_jumpElt`: `Δ_λ = Λ_λ · 1 - w_λ`.
* `Lyons.L_heatElt`: the left regular matrix of `h_t^λ` is `heatMat`.
* `Lyons.heatElt_absorb`: `h_t^λ π_G = π_G h_t^λ = π_G`.
* `Lyons.sum_heatCoeffReal`: `∑ g, p_t^λ(g) = 1`.
-/

open Matrix
-- `NormedSpace.exp`'s lemmas need the `NormedRing` instance on matrices that this scope provides.
...
omit [Group G] in
/-- **If `A` annihilates `B` on the right, so does every positive power, and the exponential
therefore fixes `B`.** -/
...
omit [Group G] in
/-- The left-handed analogue: if `B * A = 0` then `B * exp A = B`. -/
...
set_option linter.unusedDecidableInType false in
/-- The Laplacian annihilates the uniform element. -/
...
/-- **The heat element represents the heat matrix**: its entries depend only on `g * g'⁻¹` and
are real. -/
...
/-- **Absorption on the right**: `h_t^λ π_G = π_G`. -/
...
/-- **Absorption on the left**: `π_G h_t^λ = π_G`. -/
