...
Parseval's identity for `ZMod.dft`, deduced from the orthogonality relation for the standard
additive character `ZMod.stdAddChar`.

`ZMod.dft` uses the character `stdAddChar (-(j * k))`, the conjugate of the convention
`∑ j, f j * ω ^ (k * j)`. The two differ by `k ↦ -k`, which leaves every statement here invariant:
each is a sum over all of `ZMod N`, and negation permutes that sum.
...
* `ZMod.sum_norm_sq_dft`: Parseval, `∑ j, ‖f j‖ ^ 2 = (N : ℝ)⁻¹ * ∑ k, ‖𝓕 f k‖ ^ 2`.
-/
...
/-- The standard additive character takes values on the unit circle, so complex conjugation acts
on it by negating the argument. -/
...
/-- Orthogonality for the standard additive character: summing `stdAddChar (m * k)` over all `k`
gives `N` when `m = 0`, and vanishes otherwise. -/
...
/-- Parseval's identity for `ZMod.dft`, in the unnormalised convention. -/
