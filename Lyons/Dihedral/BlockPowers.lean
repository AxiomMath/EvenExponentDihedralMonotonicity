...
A self-adjoint `2 × 2` dihedral block has eigenvalues `U ± w`; raising them to `θ` and to `1 - θ`
produces the four numbers `α, β, γ, δ`, whose pairwise products satisfy two relations coming from
`μ ^ θ · μ ^ (1 - θ) = μ`.

## Main definitions

* `Lyons.bpAlpha`, `Lyons.bpBeta`, `Lyons.bpGamma`, `Lyons.bpDelta` : the four
  eigenvalue powers.

## Main results

* `Lyons.bp_nonneg` : all four are nonnegative when `U ≥ w ≥ 0`.
* `Lyons.bp_rel_diag` : `αγ + βδ = U`.
* `Lyons.bp_rel_off` : `αδ + βγ = w`.
...
/-- `γ` is `α` at the complementary exponent. -/
...
Stated for an arbitrary natural exponent `k`, not just an odd one: the proof
needs only that `t ↦ t ^ k` is monotone on `[0, ∞)`. -/
...
The diagonal of `M^θ ρ(b) M^{1-θ}` is governed by `αγ + βδ` and the off-diagonal
by `αδ + βγ`. Both collapse because `μ^θ · μ^{1-θ} = μ`. -/

section Relations
variable {U w θ : ℝ}

/-- `μ ^ θ * μ ^ (1 - θ) = μ` for a nonnegative `μ`, the degenerate case `μ = 0`
included. -/
