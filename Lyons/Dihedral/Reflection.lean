...
The exponents are written as `2 * p + 1` and `2 * p + 2`, so `m = p + 1`; the two forms are
interchangeable under `m ≥ 1`, and this one keeps truncated natural subtraction out of every
statement.

## Main results

* `Lyons.sum_coeff_split` : splitting a coefficient sum along the dihedral normal
  form.
...
No hypothesis `Ψ 0 = 0` is needed: that is required only when the sums are taken
over supports, and here they run over all of the (finite) index types, where the
reindexing is unconditional. -/
...
`Lyons.reflCoeff` are `ℂ`-valued, and the identity they satisfy is the coercion of
this real one. -/
