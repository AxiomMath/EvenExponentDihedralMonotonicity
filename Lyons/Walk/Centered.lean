...
The object the main theorem is about is `a_t^λ = h_t^λ - π_G`, and everything downstream —
fractional powers, the sandwich, the reflection inequality — needs it to be positive.

Writing `P = L_{π_G}` and `H = L_{h_t^λ}`, absorption gives `HP = PH = P`, whence
`(1 - P) H (1 - P) = H - P`. Since `1 - P` is self-adjoint, `H - P` is a `*`-conjugate of `H`,
and `H` is positive because it is the exponential of a self-adjoint matrix.
...
* `Lyons.exp_posSemidef`: the exponential of a self-adjoint matrix is positive semidefinite.
...
omit [Group G] in
/-- **The exponential of a self-adjoint matrix is positive semidefinite.** Factoring
`e^A = e^{A/2} e^{A/2}` with `e^{A/2}` self-adjoint exhibits it as `Bᴴ B`. -/
...
/-- **The centered heat element** `a_t^λ = h_t^λ - π_G`, taken as a difference of group-algebra
elements; `Lyons.co_centeredElt` identifies its coefficients with the centered transition
function. -/
...
/-- `H - P` is the `*`-conjugate of `H` by `1 - P`. -/
...
/-- **The centered heat element is self-adjoint**, positivity being defined through a positive
semidefinite — hence Hermitian — matrix. -/
