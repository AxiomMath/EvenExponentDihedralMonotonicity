...
`M = ![![U, ζw], ![conj ζ w, U]]`  with `w ≥ 0` and `|ζ| = 1`

satisfies `M = U • 1 + w • S`, where `S = ![![0, ζ], ![conj ζ, 0]]` is a self-adjoint involution.
Hence `(M - (U + w)) (M - (U - w)) = w² (S² - 1) = 0`, so the spectrum of `M` lies in the two-point
set `{U + w, U - w}`. On a two-point set `t ↦ t ^ θ` agrees with its linear interpolant, so the
`θ`-th power of `M` is again of the form `α • 1 + β • S`.

## Main definitions

* `Lyons.blockS`: the involution `![![0, ζ], ![conj ζ, 0]]`.
* `Lyons.blockM`: the dihedral block `![![U, ζ w], ![conj ζ w, U]]`.

## Main results

* `Lyons.blockM_spectrum`: the spectrum of a dihedral block lies in `{U + w, U - w}`.
* `Lyons.cfc_linear`: `cfc (fun t => c₀ + c₁ * t) a = c₀ • 1 + c₁ • a` for self-adjoint `a`.
* `Lyons.blockM_cfc_rpow`: the `θ`-th power of a dihedral block, in the form `α • 1 + β • S`.
-/
...
/-- The involution part `![![0, ζ], ![conj ζ, 0]]` of a dihedral block. -/
...
-- The closing `simp` reduces a `smul`-of-matrix identity to a disjunction whose second branch is
-- `abel`-trivial; pinning it to `simp only` would mean listing the `smul`/`sub` normalisation
-- lemmas.
...
Note that `w ≤ U` is **not** needed: the two eigenvalues `U ± w` may be arbitrary reals, and only
`w ≠ 0` matters, so that the interpolant is well defined. Positivity of the eigenvalues is instead
what makes `α` and `β` nonnegative.

The degenerate case `w = 0` **is** admitted, at the cost of one extra case: there the block is the
scalar `U`, its spectrum is `{U}`, and `bpBeta U 0 θ = 0` makes the stated formula read
`U ^ θ • 1`. -/
