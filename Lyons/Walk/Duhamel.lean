...
The derivative of `α ↦ e^{-t(A + αB)}` in the parameter `α`, for matrices `A` and `B` that need
**not** commute.

It rests on the exact, `ε`-free identity

`e^{-tA'} - e^{-tA} = -∫₀ᵗ e^{-sA'} (A' - A) e^{-(t-s)A} ds`,

which holds for *any* two matrices: no commutation between `A` and `A'`, no positivity and no
self-adjointness. Taking `A' = A + aB` and `A = A + αB`, the difference `A' - A` is exactly
`(a - α) • B`, so the identity computes the slope of the exponential in the parameter on the
nose, with no error term.

The matrix written `e^{-(t-s)A}` above is `exp ((s - t) • A)` below.

## Main results

* `Lyons.duhamel_identity`: the exact integral identity.
* `Lyons.slope_exp_param`: the slope of `a ↦ e^{-t(A + aB)}`, as an integral.
...
/-- **The interpolating path's derivative collapses to one term.** The two terms of the product
rule combine because `A` commutes with `exp ((s - t) • A)`. -/
...
/-- **Duhamel's identity.** The difference of two matrix exponentials as an integral over the
interpolating path. Exact, with no `ε` and no limit. -/
...
/-- **The slope of the exponential in its parameter, exactly.** The two generators differ by
`(a - α) • B`, so that factor cancels the denominator of the slope. -/
...
/-- **Duhamel's formula.** `A` and `B` need not commute; when they do, the integrand is constant
in `s` and this reduces to the elementary product rule. -/
