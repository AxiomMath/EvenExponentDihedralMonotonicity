...
Together with `Lyons.sum_heatCoeffReal`, this says that the coefficients of `h_t^λ` are a
probability distribution on `G`.

`Δ_λ = Λ_λ · 1 - w_λ` splits the generator into a scalar and the jump element, whose coefficients
are the rates and so are nonnegative. The scalar commutes with everything, so

`exp (-t Δ_λ) = e^{-tΛ_λ} · exp (t w_λ)`,

and every entry of `exp (t w_λ)` is a limit of partial sums of matrices with nonnegative entries.
`ComplexOrder` makes "nonnegative entry" a genuine order fact (`0 ≤ z` there means
`0 ≤ z.re ∧ z.im = 0`, so it carries realness too), and that order is closed, which is what lets
the limit inherit it.

`0 ≤ t` is load-bearing: for negative time the walk runs backwards and the coefficients go
negative.

## Main results

* `Lyons.exp_entry_nonneg`: the exponential of an entrywise-nonnegative matrix is entrywise
  nonnegative.
* `Lyons.heatMat_eq_smul_exp_jumpElt`: the heat matrix is a positive scalar times the exponential
  of the jump element.
...
With `Lyons.sum_heatCoeffReal` this says the coefficients of `h_t^λ` are a probability
distribution on `G`. -/
